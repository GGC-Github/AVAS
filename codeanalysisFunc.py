			...
			if flagCnt > 0:
				if 'ssh' in name:
					if '/etc/ssh/sshd_config' in self.fileList.keys():
						sshFlag = self.fileDataCheck('/etc/ssh/sshd_config', 1,\
													'exist', '^[\t ]*PermitRootLogin\s\S+$',\
													'PermitRootLogin')
						if sshFlag == 0:
					...
					if '/etc/pam.d/remote' in self.fileList.keys():
						secuFlag = self.fileDataCheck('/etc/pam.d/remote', 1,\
													'exist', '^[\t ]*auth\s.*pam_securetty\.so',\
													'pam_securetty.so')
					elif '/etc/pam.d/login' in self.fileList.keys():
						secuFlag = self.fileDataCheck('/etc/pam.d/login', 1, 'exist',\
													'^[\t ]*auth\s.*pam_securetty\.so',\
													'pam_securetty.so')
					else:
						self.stat.update( { '/etc/pam.d/remote' :\
											'Not Found Configuration File(!)' } )
						self.stat.update( { '/etc/pam.d/login' :\
											'Not Found Configuration File(!)' } )
						resultCnt += 1
					if secuFlag == 0:
						if '/etc/securetty' in self.fileList.keys():
							resultCnt += self.fileDataCheck('/etc/securetty', 0,\
															'!exist', '^[\t ]*pts\s+',\
															'pts')
						else:
							self.stat.update( { '/etc/seuretty' :\
												'Not Found Configuration File(!)' } )
							resultCnt += 1
	...
	def analysisFunc(self):
		...
		for name in denyFile:
			if name in self.fileList.keys():
				tallyFlag = self.fileDataCheck(name, 1, 'exist',\
...
class analysislinux008(codeanalysisBase.analysisBase):
	def analysisFunc(self):
		...
		if '/etc/shadow' in self.fileList.keys():
			...
			self.stat.update( { '/etc/shadow' : 'Not Found /etc/shadow File' } )
	...
	def analysisFunc(self):
		...
		if flagCnt > 0:
			if '/etc/mail/sendmail.cf' in self.fileList.keys():
				...
				self.stat.update( { '/etc/mail/sendmail.cf' :\
									'Not Found Configuration File(!)' } )
				self.stat.update( { '/etc/sendmail.cf' :\
									'Not Found Configuration File(!)' } )

			if dataFile is not None:
				resultCnt = self.fileDataCheck(dataFile, 1, 'exist',\
...
class analysislinux032(codeanalysisBase.analysisBase):
	def analysisFunc(self):
		...
		if flagCnt > 0:
			...
			if dataFile is not None:
				privCnt = self.fileDataCheck(dataFile, 1, 'exist',\
											'^[\t ]*O\s+PrivacyOptions\s*=\s*.+$',\
											'PrivacyOptions')
				if privCnt == 0:
					resultCnt += self.dataStrGetValue(dataFile,\
													'^[\t ]*O\s+PrivacyOptions\s*=\s*(.+$)',\
													'restrictqrun', '!')
			if resultCnt > 0:
				self.fullString[1] = '취약'
